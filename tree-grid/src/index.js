...
class TreeGrid extends React.Component {
    ...
    switch (key) {
      ...
      case LEFT:
        ...
        if (document.activeElement.tagName === "TR") {
          ...
          if (this.state.data[index].expanded === false) {
  ...
  getParent(currentRow, startRow) {
    let i = currentRow;
    while (i >= Number(startRow)) {
      if (this.state.data[i].level < this.state.data[currentRow + 1].level) {
        return i;
      }
      i--;
    }
  ...
  toggleHide(rowIndex) {
    ...
    while (currentLevel < nextLevel) {
      let parent = this.getParent(k, rowIndex);
 ...
 * 
<TreeGrid>
    <TreeGridItem level={0}>
        <TreeGridCell>!!!TreeGrids!!!</TreeGridCell>
        <TreeGridCell>This is text</TreeGridCell>
        <TreeGridCell>johnDoe@mail.com</TreeGridCell>
    </TreeGridItem>
